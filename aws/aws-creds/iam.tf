...
resource "aws_iam_policy" "earthcube_terraform_iam_policy" {
    name = "earthcube-terraform-policy"
    path = "/"
    description = "Permissions for Terraform-controlled EKS cluster creation and management"
    policy = data.aws_iam_policy_document.earthcube_terraform_iam_policy_source.json
}

resource "aws_iam_user_policy_attachment" "attach-terraform-permissions" {
  user        = aws_iam_user.user.name
  policy_arn  = aws_iam_policy.earthcube_terraform_iam_policy.arn
}
...
data "aws_iam_policy_document" "earthcube_terraform_iam_policy_source" {
	version = "2012-10-17"
